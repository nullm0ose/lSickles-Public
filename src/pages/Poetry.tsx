...
export default function Poetry() {
  // Sort poems by date descending (latest first)
  const sortedPoems = [...poetryPosts].sort(
    (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
  );

  const featuredPoem = sortedPoems[0]; // Latest poem as featured
...
const renderImage = (image?: string, altText?: string) =>
  ...
  return (
    <main className="px-6 py-16 space-y-20">
      {/* Page Header */}
      <section className="space-y-4 max-w-3xl opacity-0 animate-fade-in-up"
  ...
  <div className="relative h-80 w-full">
    ...
    {renderImage(featuredPoem.image, featuredPoem.title)}
        ...
        <div className="space-y-4">
          ...
          <h2 className="text-4xl font-serif ">{featuredPoem.title}</h2>
          <p className="text-foreground leading-relaxed">{featuredPoem.excerpt}</p>
          <Link to={`/poetry/${featuredPoem.slug}`} className="inline-block text-primary font-medium hover:underline">
            Read Poem →
          </Link>
        </div>
      </section>
    ...
    <h2 className="text-3xl font-serif  mt-5 relative inline-block">
        ...
        {recentPoems.map((poem, index) => (
          <div key={`${poem.slug}-${index}`}
               ...
               style={{ animationDelay: `${0.45 + index * 0.12}s`, animationDuration: "0.35s", animationFillMode: "forwards" }}>
    {/* Image with depth */}
